  ...
  const findNearestStallInDirection = (currentStall, currentCompassAngle) => {
    ...
    sectionKeys.forEach((secKey) => {
      ...
      stalls.forEach((stall, idx) => {
        ...
        // User specifically wants the arrow to Stall 13 to appear ON the red counter
        // The red counter is at compassAngle = 180 (South) due to the needs180Offset calibration.
        if (currentStall.id === '1(u)' && stall.id === '13(u)') {
          targetCompassAngle = 180;
        }
  ...
  return (
    <div className="relative w-full bg-black overflow-hidden font-sans select-none" style={{ height: 'calc(var(--vh, 1vh) * 100)' }}>
        ...
        <div className="relative">
          ...
          {stallDropdownOpen && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2.5 bg-white/95 backdrop-blur-md border border-black/10 rounded-2xl p-1.5 shadow-2xl z-50 flex flex-col gap-0.5 min-w-[200px] max-h-[220px] overflow-y-auto">
              <div className="px-3 pt-1 pb-0.5 text-[9px] font-black text-slate-400 uppercase tracking-wider">All Stalls in {activeSection.name}</div>
      ...
      {selectedStall && uiVisible && !detailsCollapsed && (
        <div className="absolute bottom-16 md:bottom-0 left-0 right-0 z-20 px-4 pb-4 pointer-events-none">
          <div className="max-w-4xl mx-auto relative pointer-events-auto">
            <div className="bg-white/95 backdrop-blur-md rounded-3xl border border-black/10 shadow-2xl relative overflow-hidden">
              {/* Collapse Toggle */}
              <button
